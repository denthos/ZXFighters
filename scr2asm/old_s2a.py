...
if sprite:
    ...
    with open(scr_input, "rb") as f:
        with open(new_asm_file, 'w+') as nf:
            ...
            while byte != "" and total_pixels_to_read != 0:
                ...
                while holder != "" and byteToInt(byte) == byteToInt(holder) and total_pixels_to_read != 0:
                    if throw_out_counter >= s_size or curr_row >= s_size:
                        debug2 += 1
                        if throw_out_counter >= (31): #32 - size i don't think is right #if we just moved down a row
                            throw_out_counter = 0;
                            curr_row += 1 # when we bleed over into the next row 
                            if curr_row >= 23:
                                curr_row = 0 #reset, back to the top
                            else: 
                                f.read(1)
                                total_num_bytes_read += 1
                                throw_out_counter += 1
                                total_pixels_to_read -= 1
                        else: #throw out but we are in the sprite (+1 ?)
                            throw_out_counter += 1
                            f.read(1) 
                            total_num_bytes_read += 1
                            total_pixels_to_read -= 1
                        continue
                    debug += 1
                    throw_out_counter += 1; #keep track of how many in a row we read
                    repeat_count += 1;
                    total_num_bytes_read += 1
                    total_pixels_to_read -= 1
            ...
            nf.write('\n\nattr_bytes:\n')
            nf.write(str(total_num_bytes_read) + '\n')
            nf.write(str(debug) + '\n')
            nf.write(str(debug2) + '\n')
            nf.write(str(total_num_bytes_read) + '\n')
            nf.write(str(total_pixels_to_read) + '\n')
